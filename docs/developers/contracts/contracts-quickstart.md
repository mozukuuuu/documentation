...
Let's say our target contract contains a function that _**only our source contract should be able to call**_.
...
The target contract now has to be written carefully given our authentication requirements. Remember that _**only our source contract should be able to call**_ the target function.
